...
const ShipmentToME = ({navigation, route}) => {
  const addOne = useRef();
  const addTwo = useRef();
  const citi = useRef();
  const stat = useRef();
  ...
  const fetchClick = () => {
    ...
    setFetchVisible(true);
    shipToMeApi(addressObject);
  };

  const shipToMeApi = obj => {
    //setIsLoading(false);
    setIsLoading(true);
    try {
      ...
      formdata.append('shipment_type', 8);

      // console.log(formdata);
      // return;
      Axiosinstance.post(ApiEndPoint.ship_to_me, formdata).then(
        ({ok, status, data, problem}) => {
          setIsLoading(false);
          if (ok) {
    ...
    <View
      ...
      <KeyboardAvoidingView
        ...
        <AnimationModel
          ...
          label={'Shipment requested successfully.'}
